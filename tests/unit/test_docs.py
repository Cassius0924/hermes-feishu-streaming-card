...
def test_v432_card_dialect_hotfix_release_contract():
    ...
    notes = read_doc("docs/release-notes-v4.3.2.md")
    notes_en = read_doc("docs/release-notes-v4.3.2.en.md")
    notes_v430 = read_doc("docs/release-notes-v4.3.0.md")
    notes_v430_en = read_doc("docs/release-notes-v4.3.0.en.md")
    todo = read_doc("TODO.md")
    event_flow = read_doc("docs/wiki/event-flow.md")
    acceptance = read_doc("docs/wiki/feishu-acceptance.md")

    assert 'version = "4.3.2"' in pyproject
    assert '__version__ = "4.3.2"' in package
    assert config.startswith("# Hermes Feishu Streaming Card V4.3.2")
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.3.2}"' in compose
    assert "HFC_VERSION: v4.3.2" in workflow
    assert "当前发布候选为 `4.3.2`" in readiness
    assert "Current release candidate: `4.3.2`" in readiness_en
    for text in (notes_v430, notes_v430_en):
        assert "3c27eb6234bf91b8ceee9e9071591b31e9b148cb" in text
        assert "17" in text
    ...
    for text in (notes, notes_en, readiness, readiness_en):
        assert "Issue #227" in text
        assert "230099" in text
        assert "200673" in text
        assert "schema 2.0" in text
        assert "legacy" in text
    for text in (notes, notes_en):
        assert "200800" in text
        assert "@saulgoodmanngabriel" in text
        assert "@lyp88997" in text
    assert "V4.3.2" in todo
...
def test_v400_release_docs_cover_live_runtime_cards():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.3.2" in doc
    for event_name in (
...
def test_v4021_release_docs_record_content_integrity_and_real_feishu_acceptance():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.3.2" in doc
    assert "`v4.3.2`（Compose 示例）" in guide
    assert "The Compose example defaults `HFC_VERSION` to `v4.3.2`." in guide_en
...
def test_v414_release_docs_define_manifestless_legacy_migration_candidate():
    ...
    assert "HFC_VERSION=v4.3.2" in install_doc
    assert "HFC_VERSION: v4.3.2" in workflow
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.3.2}"' in compose
