...
def test_v4210_security_lifecycle_release_contract():
    ...
    notes = read_doc("docs/release-notes-v4.2.10.md")
    notes_en = read_doc("docs/release-notes-v4.2.10.en.md")
    todo = read_doc("TODO.md")
    maintenance = read_doc("docs/wiki/maintenance-guide.md")
    playbook = read_doc("docs/wiki/release-playbook.md")

    assert 'version = "4.2.10"' in pyproject
    assert '__version__ = "4.2.10"' in package
    assert config.startswith("# Hermes Feishu Streaming Card V4.2.10")
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.10}"' in compose
    assert "HFC_VERSION: v4.2.10" in workflow
    assert "当前发布候选为 `4.2.10`" in readiness
    assert "Current release candidate: `4.2.10`" in readiness_en
    assert "hfc-sidecar-request-v1" in notes
    assert "hfc-sidecar-request-v1" in notes_en
    assert "V4.2.10" in todo
    assert "SidecarRequestProofVerifier" in notes
    assert "绝对截止时间" in maintenance
    assert "CodeQL" in playbook
...
def test_v400_release_docs_cover_live_runtime_cards():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.10" in doc
    for event_name in (
...
def test_v4021_release_docs_record_content_integrity_and_real_feishu_acceptance():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.10" in doc
    assert "`v4.2.10`（Compose 示例）" in guide
    assert "The Compose example defaults `HFC_VERSION` to `v4.2.10`." in guide_en
...
def test_v414_release_docs_define_manifestless_legacy_migration_candidate():
    ...
    assert "HFC_VERSION=v4.2.10" in install_doc
    assert "HFC_VERSION: v4.2.10" in workflow
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.10}"' in compose
