...
- `FeishuClient.send_text_message()` 现在与 card-send 路径一致：显式 `reply_in_thread=true` 但缺少 `reply_to_message_id` 时 fail-closed，拒绝请求而不是发送顶层文本。默认 `reply_in_thread=false` 保持现有兼容行为。

## 安全边界

- 只有明确的 thread intent 和可验证 `om_` reply anchor 才会触发该路径；缺 anchor 是拒绝，不是 best-effort top-level fallback。
...
- 完整 pytest、远端 CI、exact merge SHA、包构建、公开 tag/install、Release assets/checksums 与真实 Feishu/Lark 客户端验收均尚未在本 release candidate 中完成；自动化不替代真实客户端证据。
