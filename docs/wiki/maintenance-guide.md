...
- `/update` 确认必须在 ACK 前创建 durable job，再持久化 HFC drain lease，并用 Hermes 原生 external drain marker 关闭 turn/cron/API 准入；维护进程必须看到 schema v2 的 `_active_work_count()` 聚合计数、external drain 生效与连续 heartbeat，不能把缺失计数解释为零。
