...
- The final card JSON is checked through the same serializer against five rendered tables, 200 tagged elements, and a conservative 28,000-byte UTF-8 budget. A non-terminal overflow shows a small waiting card while collection continues. A terminal overflow never sends a partial answer: it hands the complete answer back to Hermes native delivery exactly once. An existing card receives only a short, non-sensitive handoff notice, and retries remain idempotent.
...
Docker Compose remains an ordinary setup, sidecar, and Gateway container stack. It pins `detached`, runs no systemd inside the image, and requests no privileged host integration.
