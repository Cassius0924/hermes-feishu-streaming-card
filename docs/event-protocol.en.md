...
| `POST /native-handoff/recover` | Exactly match a pending descriptor by obligation/content/plan hashes and canonical route | `hfc-native-handoff-recovery-v2` | Any fence mismatch returns not found; Hermes preserves its recovered marker and random-UUID fail-open path |
